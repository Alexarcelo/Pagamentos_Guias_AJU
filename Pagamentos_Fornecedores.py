...
def tratar_tipos_veiculos(df_escalas):
    ...
    dict_tp_veic = {'BUS DD': 'Bus DD', 'Executivo': 'Utilitario', 'Micrão': 'Micro', 'MICRO-ÔNIBUS': 'Micro', 'MITSUBSHI': 'Utilitario', 'Ônibus': 'Bus', 'Ônibus DD': 'Bus DD'}
...
def identificar_trf_conjugados(df_escalas_pag):
    ...
    for veiculo in df_in_out['Veiculo'].unique():
        ...
        for data_ref in df_veiculo['Data da Escala'].unique():
            ...
            if len(df_data)>1 and len(df_data['Tipo de Servico'].unique())>1:
                ...
                for index in range(1, len(df_ref), 2):
                    ...
                    regiao_2 = df_ref.at[index-1, 'Regiao']
                    ...
                    if primeiro_trf=='OUT':
                        ...
                        hora_voo_out_outros = pd.to_datetime(df_ref.at[index-1, 'Data | Horario Apresentacao'])

                        data_hora_voo_out_outros = hora_voo_out_outros.date()

                        hora_voo_in_outros = pd.to_datetime(df_ref.at[index, 'Horario Voo'], format='%H:%M:%S').replace(year=data_hora_voo_out_outros.year, month=data_hora_voo_out_outros.month, 
                    ...
                    if regiao_2=='Makai' and regiao=='Aracaju' and primeiro_trf=='OUT' and segundo_trf=='IN' and (hora_voo_in_outros - hora_voo_out_outros <= timedelta(hours=2)):

                        df_escalas_pag.at[index_1, 'Servico Conjugado'] = 'X'

                        df_escalas_pag.at[index_2, 'Servico Conjugado'] = 'X'

                    elif regiao=='Aracaju' and regiao==regiao_2 and primeiro_trf=='OUT' and segundo_trf=='IN' and (hora_voo_in_outros - hora_voo_out_outros <= timedelta(hours=1, minutes=30)):
                    ...
                    elif regiao=='Makai' and regiao==regiao_2 and primeiro_trf=='OUT' and segundo_trf=='IN' and (hora_voo_in_outros - hora_voo_out_outros <= timedelta(hours=2)):

                        df_escalas_pag.at[index_1, 'Servico Conjugado'] = 'X'

                        df_escalas_pag.at[index_2, 'Servico Conjugado'] = 'X'

                    elif regiao=='Makai' and regiao==regiao_2 and primeiro_trf=='IN' and segundo_trf=='OUT' and (hora_voo_out_outros - hora_voo_in_outros <= timedelta(hours=1, minutes=20)):
...
def ajustar_nome_fornecedor(df_escalas_pag, nome_fornecedor, palavra_chave):

    df_escalas_pag['Fornecedor Motorista'] = df_escalas_pag['Fornecedor Motorista'].astype(str)

    df_escalas_pag['Fornecedor Motorista'] = df_escalas_pag['Fornecedor Motorista'].apply(lambda x: nome_fornecedor if palavra_chave in x else x)
...
def verificar_tarifarios_especifico_fornecedor(df_escalas_pag, nome_fornecedor):
    
    df_escalas_terra_dourada = df_escalas_pag[df_escalas_pag['Fornecedor Motorista']==nome_fornecedor].reset_index(drop=True)
    ...
    if df_escalas_terra_dourada['Valor Final'].isnull().any():
        ...
        st.error(f'Os serviços {nome_servicos_sem_tarifario_terra_dourada} não estão tarifados para o Fornecedor {nome_fornecedor}. Por favor, tarife-os e tente novamente')
...
st.title('Mapa de Pagamento - Fornecedores - AJU')
...
if gerar_mapa:
    ...
    df_escalas_group = df_escalas.groupby(['Data da Escala', 'Escala', 'Veiculo', 'Tipo Veiculo', 'Servico', 'Tipo de Servico', 'Fornecedor Motorista', 'Motorista'])\
        .agg({'Horario Voo': 'first', 'Data | Horario Apresentacao': 'max'}).reset_index()
...
if 'df_pag_final_forn' in st.session_state:
    ...
    if data_pagamento:
        ...
        if fornecedor and data_pagamento and data_inicial and data_final and fornecedor[0]!='SELECIONAR TODOS':
        ...
        elif fornecedor and data_pagamento and data_inicial and data_final and fornecedor[0]=='SELECIONAR TODOS':
            ...
            with row2_1[1]:
                ...
                if enviar_informes_financeiro:
                    ...
                    for fornecedor_ref in lista_fornecedores:
                        ...
                        lista_htmls.append([html_content_fornecedor_ref, '79981230294'])
        ...
        st.warning('Por favor, preencha o campo de Data de Pagamento p/ visualizar os mapas de pagamentos')

if 'html_content' in st.session_state and len(fornecedor)==1 and 'df_pag_final_forn' in st.session_state and data_pagamento:
